...
- 条目是只追加的历史。要更正一条写错的条目，用后续条目说明错在哪；不要重写别人
  可能已经引用过的记录。
- 带日期的检查点是账本条目，不是 RFC 小节。RFC 正文不得再出现含 `checkpoint` /
  `检查点` 的标题；原位置只留一行指针（`检查点已移至执行账本：<指向 ledger/<rfc-slug>/<file> 的链接>`）。
  事后从 RFC 搬入的小节以一行来源说明开头，写明源 RFC、原小节标题、搬迁日期和
  RFC 基线 commit，读者可以据此 diff 这次搬迁。
