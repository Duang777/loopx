...
| Field | Receipt |
| --- | --- |
| Canonical owner | Before: Python owned terminal admission and archive retention, while completion reduction and lease operations crossed narrower TS boundaries. After: `todo_terminal_decision.ts`, `todo_terminal_lifecycle.ts`, and `todo_archive_selection.ts` are the typed owners of terminal admission, lease release, completion reduction, successor validation, CAS, receipt replay, and archive selection. The same archive selector is imported by the native transaction and called once per legacy Markdown archive batch. |
| Legacy semantic code deleted | 74 Python product LOC are removed from semantic ownership: the 49-line terminal decision and the 25-line archive eligibility/order/standing-receipt selector. The remaining Python complete/supersede bodies are the unpromoted compatibility writer, not a second promoted authority. Other deleted lines are adapter reshaping and moves and are not counted as payoff. |
| Bridge code added | 732 gross product LOC are classified as bounded transport/compatibility: 514 lines of `provider_terminal_lifecycle.py` outside the 111-line successor-proposal derivation, 171 lines in the local TS request decoder/router, 33 lines in the legacy archive result adapter, 8 handler-registration lines, and 6 projection-settlement lines. The 29-line `resolve_todo_state_path` extraction is a move, not payoff. Host-local validation declaration storage/execution is a retained external effect and is not mislabeled as bridge deletion. |
| Successor ownership | The public caller owns requested successor text and options. Python `_successor_record`/`_build_successors` currently own 111 LOC of compatibility-time business derivation: inherited priority, capability binding, user binding, exclusions, and `unblocks_todo_id`; JSON record construction is transport. TypeScript validates the complete proposal and atomically commits it, but this derivation remains an explicit facade-exit item rather than being claimed as deleted authority. |
| Cross-runtime calls | Measured at the public facade: promoted complete without validation, supersede, and archive use three request/responses (`todo_list`, terminal/archive transaction, projection readback). A validated complete uses four (`todo_list`, terminal preflight, terminal finalization, projection readback) plus one declared host-local validation effect. After an injected post-commit projection crash, the first attempt uses two calls and the receipt replay uses three. The promoted happy path did not exist before this cutover; the legacy terminal path adds one TS decision call while it remains supported. |
| Product-code net change | Final merge-base classification is +3,254/−145 product LOC, net +3,109; tests/fixtures/examples are +2,956/−154, net +2,802; generated contracts are +3/−0 and docs are excluded. The increase delivers a complete provider-neutral transaction, real File/PostgreSQL conformance, public facade parity, and durable mutation gates; it is not counted as deletion payoff. |
...
| Facade exit | Delete `provider_terminal_lifecycle.py`, its Python call-count tests, and `resolve_todo_state_path` after the top-level Todo CLI runs in-process TypeScript, registry/lifecycle facts arrive through the native caller, validation is executed by a native host adapter, typed successor intent replaces `_build_successors`, and compatibility consumers drain the canonical journal directly. Delete the legacy `todo.archive.select` crossing when the default Markdown archive writer migrates. |
| Correctness evidence | Independent archive-order/standing-receipt semantics kill an oldest-selection mutant; optional `note`/`evidence`/`reason` cover `None`, empty, ordinary, Python-Unicode-whitespace-only, and whitespace compaction before/after promotion; Stage 2C proves provider-first fence routing, live-lease import, orphan-history filtering, management-lock exclusion, replay, and zero-write previews. File and real PostgreSQL providers execute the same terminal conformance, while the independent legacy arm remains mandatory compatibility evidence. |
