...
def handle_todo_continuation(args, *, registry_path, runtime_root_arg, output_format, print_payload):
    ...
    try:
        ...
        request = {
            "runtime_root": str(root), "goal_id": args.goal_id, "todo_id": args.todo_id,
            "agent_id": args.agent_id, "registered_agents": registered_agent_ids_from_registry(registry_path, args.goal_id),
            "action": args.action, "session_id": args.session_id,
            "operation_id": args.operation_id, "expected_provider_revision": args.expected_revision,
            "workspace": str(Path(args.workspace).expanduser().resolve()), "artifacts": args.artifact,
        ...
        payload = {"ok": False, "status": "failed", "reason": str(exc)}
